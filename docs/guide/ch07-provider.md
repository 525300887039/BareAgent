...
统一抽象定义在 `src/bareagent/provider/base.py`。
...
`AnthropicProvider` 位于 `src/bareagent/provider/anthropic.py`，是对 `anthropic.Anthropic` 客户端的轻量封装。
...
`OpenAIProvider` 位于 `src/bareagent/provider/openai.py`。它除了支持官方 OpenAI API，也被用来承载 DeepSeek 这类 OpenAI 兼容接口。
...
provider 的统一入口是 `src/bareagent/provider/factory.py` 中的 `create_provider(config)`。
