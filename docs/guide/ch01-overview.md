...
| 模块 | 路径 | 职责 |
|------|------|------|
| 入口与 REPL | `src/bareagent/main.py` | 命令行入口、交互式 REPL 循环、斜杠命令处理 |
| 核心循环 | `src/bareagent/core/` | agent_loop、工具注册与 Schema、处理器、系统提示组装 |
| LLM 提供商 | `src/bareagent/provider/` | 抽象基类、Anthropic/OpenAI 实现、工厂创建 |
| 权限守卫 | `src/bareagent/permission/` | 四种权限模式、危险命令检测、allow/deny 规则 |
| 消息与会话 | `src/bareagent/memory/` | 消息压缩（微压缩 + LLM 摘要）、token 估算、会话转录 |
| 规划与委派 | `src/bareagent/planning/` | 智能体类型、子智能体委派、任务管理、TODO、技能系统 |
| 多智能体 | `src/bareagent/team/` | 消息总线、协议状态机、自治智能体、队友管理 |
| 后台执行 | `src/bareagent/concurrency/` | 后台任务管理、完成通知 |
| 终端 UI | `src/bareagent/ui/` | 基于 rich 的控制台输出、流式打印 |
