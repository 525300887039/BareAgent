...
def test_mailbox_send_receive(tmp_path):
    ...
    bus.send(Message(
        id="", from_agent="agent_a", to_agent="agent_b",
        content="你好", msg_type="chat", timestamp=""
    ))
...
def test_mailbox_isolation(tmp_path):
    ...
    bus = MessageBus(mailbox_dir=tmp_path / ".mailbox")
    bus.send(Message(
        id="", from_agent="a", to_agent="b",
        content="给 b 的消息", msg_type="chat", timestamp=""
    ))
    bus.send(Message(
        id="", from_agent="a", to_agent="c",
...
def test_mailbox_agent_name_validation(tmp_path):
    ...
    try:
        bus.send(Message(
            id="", from_agent="../evil", to_agent="target",
            content="payload", msg_type="chat", timestamp=""
        ))
